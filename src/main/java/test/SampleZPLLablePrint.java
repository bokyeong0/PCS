...
public class SampleZPLLablePrint {
	...
	public static StringBuffer assetLabelDiversion() {
		...
		EgovMap printingInfo = new EgovMap();
		
		String labelState = "O"; // 라벨상태 (F,L,O)
		String shortOrderNo = "009"; // 단축주문번호 (3자리)
		String trayNo = "30"; // 트레이번호 (유동)
		String pickId = "123456789012"; // 트레이번호 (유동)
		String orgOrderNo = "20161024-000154"; // 원주문번호 ( [9자리]-[6자리])
		String deliveryCount = "1"; // 회차
		String zoneCode = "Aa"; // Zone (2자리 | AA(후방/상온), AB(후방/냉장), AB(후방/냉동), AD(후방/대용량), BA(매장/상온), BB(매장/냉장), BC(매장/냉동),  BD(매장/대용량), CA(대면/축산) )
		String deliveryDate = "20160803"; // 배송일
		String deliveryAreaName = "일산동구"; // 배송구명
		String customerName = "김*희"; // 고객명
		String labelStatusFullName = "";
		String trolleyId = "000000000001";

		printingInfo.put("labelState", labelState);
		printingInfo.put("shortOrderNo", shortOrderNo);
		printingInfo.put("trayNo", trayNo);
		printingInfo.put("pickId", pickId);
		printingInfo.put("orgOrderNo", orgOrderNo);
		printingInfo.put("deliveryCount", deliveryCount);
		printingInfo.put("zoneCode", zoneCode);
		printingInfo.put("deliveryAreaName", deliveryAreaName);
		printingInfo.put("customerName", customerName);
		printingInfo.put("deliveryDate", deliveryDate);
		printingInfo.put("trolleyId", trolleyId);
		...
		assetSb.append("^FO500, 80^AHR,170,150^FD" + printingInfo.get("shortOrderNo") + "^FS ");
		assetSb.append("^FO500, 360^AHR,110,100^FD"  + "-" + printingInfo.get("trayNo") + "^FS ");

		// 바코드 2차
		assetSb.append("^FO590, 650^BY4^B2R,150,Y,N,N^FD" + printingInfo.get("pickId") + "^FS  바코드용 2차");
		assetSb.append("^FO450, 100^AHR,50,45^FD[" + printingInfo.get("orgOrderNo") + "]^FS ");
		...
		assetSb.append("^FO220, 580^AHR,150,140^FD" + printingInfo.get("customerName") + "^FS ");
...
/*		
		...
		assetSb.append("^CWH,E:KFONT3.FNT^FS  (폰트명) ");
		assetSb.append("^CI28 ");
		assetSb.append("^LH190,0 ");		
		assetSb.append("^FO700, 80^AHR,50,45^FD" + sData1 + "^FS ");
		assetSb.append("^FO500, 80^AHR,170,150^FD" + sData2 + "-" + sData3 + "^FS ");

		// 바코드 2차
		assetSb.append("^FO550, 650^BY4^B2R,150,Y,N,N^FD" + sData10 + "^FS  바코드용 2차");
		assetSb.append("^FO450, 100^AHR,50,45^FD[" + sData4 + "]^FS ");
		assetSb.append("^FO210, 80^AHR,170,130^FD" + sData5 + sData11 + "-"	+ sData6 + "^FS ");
		assetSb.append("^FO310, 580^AHR,150,140^FD" + sData8 + "^FS ");		
		assetSb.append("^FO140, 580^AHR,150,140^FD" + sData9 + "^FS ");		
		assetSb.append("^FO135, 100^AHR,60,50^FD"
